...
async function syncSegmentStatuses(
    ...
    userId: string
): Promise<{ segments: LongVideoSegment[]; updated: boolean }> {
    ...
    return { segments: updatedSegments, updated };
}
...
export async function GET(req: NextRequest) {
    try {
        ...
        const syncResult = await syncSegmentStatuses(jobId, segments, user.id);
        ...
        const response = {
            ...
            canContinue: job.status === "processing" && 
